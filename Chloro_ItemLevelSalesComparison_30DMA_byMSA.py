...
OT_query =  """SELECT RIGHT('00000'+ISNULL(zip.fips,''),5) as fips, zip.msa, Truly
FROM
...
(SELECT CASE WHEN tmsa IS NOT NULL THEN tmsa
						 WHEN tmsa IS NULL THEN msa
						 END AS msa,
						 ((ISNULL(truly_ttl,0.000000000000000001))-(ISNULL(whiteclaw_ttl,0.000000000000000001))) as Truly

FROM (SELECT fips as tmsa, PQ_DMA30 as truly_ttl
            FROM
...
(SELECT fips as msa, PQ_DMA30 as whiteclaw_ttl
            FROM
            (SELECT agg_daily.sector_type,
                    --(date_trunc('week', cur_orderdate - interval '6d') + interval '6d')::date as oweek,
                    fips,
...
# Create Chloropleth
chloro_salescomp_fig = px.choropleth(df_raw, geojson=counties, locations='fips', color='truly',
